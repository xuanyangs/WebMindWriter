const fanqiePuaMap: Record<number, string> = {
...
export function decodeFanqieText(value: string | undefined): string | undefined {
  if (value === undefined) return undefined;
  return [...value].map((char) => fanqiePuaMap[char.codePointAt(0) ?? -1] ?? char).join("");
}
